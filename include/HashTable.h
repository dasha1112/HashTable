...
public:
	...
	THashNode<TKEY, TVAL>& operator [](TKEY* k);
	THashNode<TKEY, TVAL>* Find(TKEY* k);
...
template<class TKEY, class TVAL>
inline THashNode<TKEY, TVAL>& THashTable<TKEY, TVAL>::operator[](TKEY* k)
{
	int index = Hash(*k);
...
template<class TKEY, class TVAL>
inline THashNode<TKEY, TVAL>* THashTable<TKEY, TVAL>::Find(TKEY* k)
{
	int index = Hash(*k);
	THashNode<TKEY, TVAL>* curr_item = data[index];
	...
	while (curr_item != nullptr)
	{
		...
		{
			return curr_item; // Возвращаем узел, если ключи совпадают
		}
		curr_item = curr_item->GetNext();
	}
